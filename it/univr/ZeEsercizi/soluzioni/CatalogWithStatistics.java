...
public class CatalogWithStatistics extends Catalog {
	...
	/**
	 * Si comporta come il toString() di Catalog, ma alla fine aggiunge una riga del tipo:
	 * "This catalog contains paper books for a total of XXX pages and audiobooks for a total of YYY minutes"
	 */
	@Override
	public String toString() {
		int totPages = 0, totMinutes = 0;
		for (Book book: this)
			if (book instanceof PaperBook pBook)
				totPages += pBook.getPages();
			else if (book instanceof AudioBook aBook)
				totMinutes += aBook.getMinutes();

		return super.toString() +
			"This catalog contains paper books for a total of "
			+ totPages
			+ " pages and audiobooks for a total of "
			+ totMinutes
			+ " minutes";
	}
}