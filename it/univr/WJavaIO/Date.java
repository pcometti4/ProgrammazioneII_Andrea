...
public abstract class Date implements Comparable<Date> {
	...
	public Date(Scanner scanner) throws IOException {
		try {
	...
	public final void dumpAsText(String fileName) throws IOException {
	...
	private void verify() {
		if (month < 1 || month > 12 || day < 1 || day > daysInMonth())
			throw new IllegalDateException("data illegale!");
	}
	...
	public Season getSeason() {
		if (this.compareTo(mk(21, 3, year)) < 0)
			return Season.WINTER;
		else if (this.compareTo(mk(21, 6, year)) < 0)
			return Season.SPRING;
		else if (this.compareTo(mk(23, 9, year)) < 0)
			return Season.SUMMER;
		else if (this.compareTo(mk(21, 12, year)) < 0)
	...
	/**
	 * Restituisce una data dello stesso tipo
	 * di quella su cui è chiamato.
	 * 
	 * @param day
	 * @param month
	 * @param year
	 * @return la data
	 */
	protected abstract Date mk(int day, int month, int year);
}
