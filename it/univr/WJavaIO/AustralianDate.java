...
public class AustralianDate extends Date {
	...
	protected Date mk(int day, int month, int year) {
		return new AustralianDate(day, month, year);
	}

	public Season getSeason() {
		if (this.compareTo(mk(1, 3, getYear())) < 0)
			return Season.SUMMER;
		else if (this.compareTo(mk(1, 6, getYear())) < 0)
			return Season.AUTUMN;
		else if (this.compareTo(mk(1, 9, getYear())) < 0)
			return Season.WINTER;
		else if (this.compareTo(mk(1, 12, getYear())) < 0)
